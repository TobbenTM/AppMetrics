...
BEGIN TRY
	BEGIN TRANSACTION
		IF NOT EXISTS(SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = N'dbo' AND TABLE_NAME = @TableName)
		BEGIN
			CREATE TABLE [dbo].[@TableName] (
				...
				[Value]			DECIMAL					NULL,		-- The value of the metrics
				[Tags]			NVARCHAR(MAX)			NULL,		-- The metric's tags
				CONSTRAINT [PK_@TableName] PRIMARY KEY CLUSTERED ([Id] ASC),
				CONSTRAINT [JSON_@TableName_Tags] CHECK(ISJSON([Tags])=1),
			);
		END
	COMMIT TRANSACTION
END TRY
